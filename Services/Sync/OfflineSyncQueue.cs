...
{
    /// <summary>
    /// 未同期（オフライン中に変更された）ノートを永続的なキューとして管理する。
    /// iOS版 OfflineSyncQueue.swift に対応。
    /// </summary>
    public class OfflineSyncQueue
    {
        ...
        /// <summary>
        /// 指定したノートが未同期キューに存在するか（ConflictManagerが競合検出に使用）
        /// </summary>
        public bool HasPending(string noteName, string subFolder = null)
        {
            try
            {
                var notes = GetUnsynchronizedNotes();
